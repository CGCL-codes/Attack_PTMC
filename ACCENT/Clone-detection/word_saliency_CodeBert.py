...
def return_bleu(args, model, eval_dataset, prefix="",  best_threshold=0 ):
    query_cnt = 0
    orig_prob=[]
    orig_label=[]
...
def computer_best_substitution(model,code,code2,lable,variable_list,nearest_k_dict,args, tokenizer, pool, device, epoch, count):
    ...
    # old_lable, tmp_cnt,orig_prob, orig_label= return_bleu(args, model, eval_dataset,  best_threshold=0.5)
    tmp_cnt,orig_prob, orig_label= return_bleu(args, model, eval_dataset,  best_threshold=0.5)
    query_cnt += tmp_cnt
    ...
    for var in variable_list:  # variable list  for one code
        ...
        for new_var in nearest_k:
            ...
            tmp_cnt ,orig_prob_, orig_label_= return_bleu(args, model, eval_dataset,  best_threshold=0.5)
            query_cnt += tmp_cnt
