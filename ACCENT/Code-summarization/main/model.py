...
class Code2NaturalLanguage(object):
    ...
    def __str__(self):
            return 'model [args=%s\n , src_dict=%s\n , args.src_vocab_size = %s\n , tgt_dict = %s\n , args.tgt_vocab_size = %s\n]'%(self.args,self.src_dict,self.args.src_vocab_size,self.tgt_dict,self.args.tgt_vocab_size)
    ...
    def predict(self, ex, replace_unk=False):
        ...
        return predictions, targets, decoder_out['copy_info'],decoder_out['memory_bank']

    '''

    def predict_for_attention(self, ex, replace_unk=False):
        """Forward a batch of examples only to get predictions.
        Args:
            ex: the batch examples
            replace_unk: replace `unk` tokens while generating predictions
    ...
    def predict_for_attention(self, ex, replace_unk=False):
        ...
        #print(code_word_rep.shape)  torch.Size([1, 16])
        ...
        targets = [summ for summ in ex['summ_text']]
        return predictions, targets, decoder_out['copy_info'],decoder_out
