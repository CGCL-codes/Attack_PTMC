...
def rank_variable(code,summary,variable_list,nearest_k_dict,vocab,embeddings,max_token,
    ...
    best_substitution_list, tmp_cnt, new_variable_list =computer_best_substitution(model,code,summary,variable_list,nearest_k_dict,args, tokenizer, device, epoch,count)

    word_saliency_list = computer_word_saliency_cos(model_encoder, code, summary, new_variable_list, vocab,embeddings,max_token,
...
def prepare():
    ...
    args.load_model_path = "./saved_models/checkpoint-best-bleu/pytorch_model.bin"
    ...
    model = Seq2Seq_(encoder=encoder, decoder=decoder, config=config,
...
if __name__=='__main__':
    ...
    for mycnt in range(start_num, end_num):
        ...
        for code,summ in zip(f_code,f_summary):
            ...
            print("rank_variable")
            descend_variable_dict, f_tmp_cnt=rank_variable(code,summ,variable_list,nearest_k_dict,vocab,embeddings,max_token,
                                                model_encoder,vocab_src,vocab_trg,max_token_src,max_token_trg)
            t1 = time.time()
            query_cnt += f_tmp_cnt
        ...
        data_descend_best_var.to_pickle(
            ...
            data_root + '/data1/java' + str(mycnt) + '/test_rnn_data_descend_best_var_12_15.pkl')
        print('end...')
        print(time.strftime(' %Y-%m-%d %H:%M:%S',time.localtime(time.time())))
