...
def train_vocab(size):
    ...
    if not os.path.exists( '../Code-summarization/encoder/vocab/train1'):
        os.mkdir('../Code-summarization/encoder/vocab/train1')

    w2v = Word2Vec(corpus, vector_size=size, workers=16, sg=1, min_count=3)
    w2v.save('../Code-summarization/encoder/vocab/train1/node_w2v_code_' + str(size))
    # w2v.save('../Code-summarization/encoder/vocab/train1/node_w2v_summ_' + str(size))
