...
def main():
    ...
    url_to_code = {}
    index = 0
    with open('../../dataset/Code-summarization/test.jsonl') as f:
        for line in f:
            item = {}
            line = line.strip()
            js = json.loads(line)
            item["idx"] = index
            item["func"] = " ".join(js["code_tokens"])
            eval_data.append(item)
            index += 1
    file = "data/test.csv"
    df = pd.read_csv(file, encoding="utf-8")
    code_list = df["code"].tolist()
    idx_list = range(len(code_list))
    All_list = df["All_identifier"].tolist()

    store_path = "data/data_subs_test.jsonl"
        ...
        for item in tqdm(eval_data):
            ...
            for tgt_word in names_positions_dict.keys():
                ...
                for masked_token in masked_token_list:
                    ...
                    for i in range(n):
                        masked_token_part = masked_token[i * 200: (i + 1) * 100]
