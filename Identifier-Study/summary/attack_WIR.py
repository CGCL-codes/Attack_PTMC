...
import copy
import torch
from attacker import WIR_Attacker, eval_bleu
import logging
import random
import utils
...
def main():
    ...
    print(len(code_list), len(substitutes))
    ...
    bleu_file = "wir_" + args.statement
    if os.path.exists(bleu_file) is False:
        os.makedirs(bleu_file)
    attacker = WIR_Attacker(args, model, tokenizer, tokenizer_mlm, token2id, id2token, bleu_file)

    attack_succ_1 = 0
    attack_succ_3 = 0
    # recoder_1 = Recorder_summary("result/attack_wir_" + args.statement + "_1.csv")
    # recoder_3 = Recorder_summary("result/attack_wir_" + args.statement + "_3.csv")
    total_cnt = 0
    for index, example in enumerate(eval_examples):
        print("Index: ", index)
        original_bleu1, pre_summary, ref_summary = eval_bleu(args, [example], model, tokenizer, bleu_file)
        original_bleu = original_bleu1[0]
        if original_bleu == 0:
            continue
        code = example.source
        ...
        total_cnt += 1
        if len(statements) == 1:
            result = attacker.wir_random_attack(original_bleu, statements, example)
            if result[0] == 1:
                attack_succ_1 += 1
                attack_succ_3 += 1
            ...
            result_1 = attacker.wir_random_attack(original_bleu, statements_1, example)
            if result_1[0] == 1:
                attack_succ_1 += 1
                attack_succ_3 += 1
                flag = 1
            ...
            if flag == 0:
                ...
                result_3 = attacker.wir_random_attack(original_bleu, statements_3, example)
                if result_3[0] == 1:
                    attack_succ_3 += 1
