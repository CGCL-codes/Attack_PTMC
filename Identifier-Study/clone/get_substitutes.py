...
from utils import _tokenize, get_identifier_posistions_from_code, get_masked_code_by_position, \
    get_substitues, is_valid_variable_java
from transformers import (RobertaForMaskedLM, RobertaConfig, RobertaForSequenceClassification, RobertaTokenizer, pipeline)
...
def main():
    ...
    with open('../../dataset/Clone-detection/data.jsonl') as f:
        for line in f:
            item = {}
            line = line.strip()
            js = json.loads(line)
    ...
    eval_data1 = eval_data[0:4000]
    eval_data2 = eval_data[4000:len(eval_data)]

    store_path = "data/data_subs_test2.jsonl"
    with open(store_path, "w") as wf:
        for item in tqdm(eval_data2):
            identifiers = ""
            try:
            ...
            for tgt_word in names_positions_dict.keys():
                ...
                for masked_token in masked_token_list:
                    n = int(len(masked_token) / 100)
                    for i in range(n):
                        masked_token_part = masked_token[i * 100: (i + 1) * 100]
                ...
                for tmp_substitue in all_substitues:
                    if tmp_substitue.strip() in variable_names:
                        continue
                    if not is_valid_variable_java(tmp_substitue.strip()):
                        continue
                    try:
