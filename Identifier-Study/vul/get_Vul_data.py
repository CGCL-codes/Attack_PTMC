...
# test_idxs = []
# with open("../../dataset/Vulnerability-detection/test.jsonl") as f:
#     for line in f:
#         line = line.strip()
#         js = json.loads(line)
...
for i in range(len(code_list)):
    ...
    method_cani.append([label_list[i], idx_list[i], code_list[i], method_state]) if len(method_state) > 0 else method_cani
    Return_cani.append([label_list[i], idx_list[i], code_list[i], return_state]) if len(return_state) > 0 else Return_cani
    If_cani.append([label_list[i], idx_list[i], code_list[i], if_state]) if len(if_state) > 0 else If_cani
    Throw_cani.append([label_list[i], idx_list[i], code_list[i], throw_state]) if len(throw_state) > 0 else Throw_cani
    Try_cani.append([label_list[i], idx_list[i], code_list[i], try_state]) if len(try_state) > 0 else Try_cani
    For_cani.append([label_list[i], idx_list[i], code_list[i], for_state]) if len(for_state) > 0 else For_cani
...
for state in ["Method", "Return", "If", "Throw", "Try", "For"]:
    ...
    writer.writerow(["target", "idx", "code", "identifier"])
    tmp_list = []
    if state == "Method":
