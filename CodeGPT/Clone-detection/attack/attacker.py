...
def get_transfered_code(code):
    ...
    if os.path.exists(tmp_file):
        os.remove(tmp_file)
    jar_path = "StyleTransformer1.jar"
...
class Style_Attacker():
    ...
    def style_attack(self, code_pair, true_label, example, adv_codes,total_times):
        code_1 = code_pair[2]
        code_2 = code_pair[3]
        code_2 = " ".join(code_2.split())
        words_2 = self.tokenizer_tgt.tokenize(code_2)
        ...
        for adv_code in adv_codes:
            ...
            tmp_feature = convert_examples_to_features(tmp_code, words_2, true_label, None, None, self.tokenizer_tgt, self.args, None)
            ...
            if total_times + query_times >= 500:
                break
        ...
        #
        #     tmp_feature1 = convert_examples_to_features(tmp_code1, words_2, true_label, None, None, self.tokenizer_tgt, self.args, None)
        #     new_dataset = CodeDataset([tmp_feature1])
        #     logits, preds = self.model_tgt.get_results(new_dataset, self.args.eval_batch_size)
        #     query_times += 1
        #     if preds[0] != true_label:
        #         is_success = 1
        ...
        # for adv_code2 in adv_codes2:
        #     tmp_code2 = ' '.join(adv_code2.split())
        #     tmp_code2 = self.tokenizer_tgt.tokenize(tmp_code2)
        #
        #     tmp_feature2 = convert_examples_to_features(tmp_code2, words_2, true_label, None, None, self.tokenizer_tgt,
        #                                                 self.args, None)
        #     new_dataset = CodeDataset([tmp_feature2])
        #     logits, preds = self.model_tgt.get_results(new_dataset, self.args.eval_batch_size)
        #     query_times += 1
        #     if preds[0] != true_label:
        #         is_success = 1
        #         print("%s SUC! (%.5f => %.5f)" % \
        #               ('>>', true_label, preds[0]), flush=True)
        #         return is_success, adv_code2, query_times

        return is_success, adv_codes1, query_times
