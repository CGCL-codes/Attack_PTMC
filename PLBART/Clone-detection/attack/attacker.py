...
from utils import PLBARTCodeDataset, is_valid_identifier, get_code_tokens
...
def compute_fitness(chromesome, words_2, codebert_tgt, tokenizer_tgt, orig_prob, orig_label, true_label, code,
    ...
    new_dataset = PLBARTCodeDataset([new_feature])
...
def get_importance_score(args, example, code, code_2, words_list: list, variable_names: list,
    ...
    for index, code1_tokens in enumerate([words_list] + masked_token_list):
        new_feature = convert_examples_to_features(' '.join(code1_tokens), code_2, example[3].item(), None, None, tokenizer,
                                                   args, None)
        new_example.append(new_feature)

    new_dataset = PLBARTCodeDataset(new_example)
    # 3. 将他们转化成features
    ...
    def ga_attack(self, example, substitutes, code, initial_replace=None):
        ...
        for tgt_word in variable_substitue_dict.keys():
            ...
            if initial_replace is None:
                ...
                for a_substitue in variable_substitue_dict[tgt_word][:30]:
                    ...
                    # 记录下这次换的是哪个substitue
                    temp_replace = get_example(code_1, tgt_word, a_substitue)
        ...
        for i in range(max_iter):
            ...
            if len(feature_list) == 0:
                continue
            new_dataset = PLBARTCodeDataset(feature_list)
...
class Style_Attacker():
    def __init__(self, args, model_tgt, tokenizer_tgt, tokenizer_mlm) -> None:
        self.args = args
        self.model_tgt = model_tgt
        self.tokenizer_tgt = tokenizer_tgt
        self.tokenizer_mlm = tokenizer_mlm

    def style_attack(self, code_pair, true_label, example, adv_codes):
        code_1 = code_pair[2]
        code_2 = code_pair[3]
        code_2 = " ".join(code_2.split())
        ...
        for adv_code in adv_codes:
            ...
            #     if tmp_code.strip().split("(")[0] == code_1.strip().split("(")[0]:
            #         adv_codes1.append(tmp_code)
            tmp_code = ' '.join(adv_code.split())
            # tmp_code = self.tokenizer_tgt.tokenize(tmp_code)

            tmp_feature = convert_examples_to_features(tmp_code, code_2, true_label, None, None, self.tokenizer_tgt, self.args, None)
        ...
        for adv_code1 in adv_codes1:
            ...
            # tmp_code1 = self.tokenizer_tgt.tokenize(tmp_code1)

            tmp_feature1 = convert_examples_to_features(tmp_code1, code_2, true_label, None, None, self.tokenizer_tgt, self.args, None)
            new_dataset = PLBARTCodeDataset([tmp_feature1])
            logits, preds = self.model_tgt.get_results(new_dataset, self.args.eval_batch_size)
            query_times += 1
            if preds[0] != true_label:
                is_success = 1
                print("%s SUC! (%.5f => %.5f)" % \
                      ('>>', true_label, preds[0]), flush=True)
                return is_success, adv_code1, query_times
...
class MHM_Attacker():
    ...
    def mcmc_random(self, example, substituions, tokenizer, code_pair, _label=None, _n_candi=30,
        ...
        for iteration in range(1, 1 + _max_iter):
            # 这个函数需要tokens
            res = self.__replaceUID_random(code_2=code_2, _tokens=code_1, _label=_label, _uid=uid,
    ...
    def __replaceUID_random(self, code_2, _tokens=[], _label=None, _uid={}, substitute_dict={},
