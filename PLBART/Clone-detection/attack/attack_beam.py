    ...
    for index, example in enumerate(eval_dataset):
        ...
        if result["succ"] == 1:
            ...
            recoder.write(index, first_code, result["adv_code"], len(orig_code_tokens), len(identifiers),
                          result["replace_info"], model.query - query_times, example_end_time, "Beam")

            # new_feature = convert_examples_to_features(result["adv_code"], code_2, example[3].item(), None, None,
            #                                            tokenizer, args, None)
            #
