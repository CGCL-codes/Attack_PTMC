...
from attacker import ALERT_Attacker, read_examples
...
def main():
    ...
    if args.model_name_or_path:
        ...
        print("{} - reload model from {}".format(datetime.now().strftime('%Y-%m-%d %H:%M:%S'), args.model_name_or_path))
    ...
    assert len(eval_examples) == len(generated_substitutions)
    bleu_file = "alert"
    if os.path.exists(bleu_file) is False:
        os.makedirs(bleu_file)

    recoder = Recorder(args.csv_store_path)
    attacker = ALERT_Attacker(args, model, tokenizer, tokenizer_mlm, bleu_file)
    success_attack = 0
    total_cnt = 0
    query_times = 0
    ...
    for index, example in enumerate(eval_examples):
        ...
        substitute = generated_substitutions[index]
        identifiers = list(substitute.keys())
        ...
        is_success, adv_code, adv_summary, nb_changed_var, nb_changed_pos, replaced_words = attacker.greedy_attack(
            original_bleu, example, substitute, code)
        if is_success == -1 and args.use_ga:
            is_success, adv_code, adv_summary, nb_changed_var, nb_changed_pos, replaced_words = attacker.ga_attack(
                original_bleu, example, substitute, code, initial_replace=replaced_words)
